...
async function inlineResources() {
    const htmlPath = path.resolve('isolines-leaflet/src/demo.html');
    const outputPath = path.resolve('isolines-leaflet/demo_combined.html');

    try {
        const html = await inlineSource(htmlPath, {
            compress: false,
            rootpath: path.resolve('isolines-leaflet/src'),
            ignore: []
        });
